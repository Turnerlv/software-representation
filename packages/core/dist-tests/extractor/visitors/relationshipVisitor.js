 ...
 */
export function visitRelationship(node, sourceFile, getEvidence, nextId, repoRoot = '', sourceId = '') {
    ...
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
        ...
        let currentExpr = node.expression.expression;
        while (currentExpr) {
        ...
        if (rootIdentifier) {
            ...
            const callEvidence = { ...getEvidence(node), evidenceRole: 'syntax-call' };
            const evidences = [callEvidence];
