 ...
 */
export function extractExpressRoute(node, getEvidence, nextId) {
 ...
 */
export function extractExpressRouterMount(node, sourceFile, getEvidence, nextId) {
 ...
 */
export function extractExpressRouteParameter(node, getEvidence, nextId) {
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
        const methodName = node.expression.name.text;
 ...
 */
export function extractExpressContentNegotiation(node, getEvidence, nextId) {
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
        const methodName = node.expression.name.text;
 ...
 */
export function extractExpressResponseConnector(node, getEvidence, nextId) {
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
        const methodName = node.expression.name.text;
 ...
 */
export function extractExpressAppListen(node, getEvidence, nextId) {
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
        const methodName = node.expression.name.text;
 ...
 */
export function extractExpressResponseCookie(node, getEvidence, nextId) {
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
        const methodName = node.expression.name.text;
