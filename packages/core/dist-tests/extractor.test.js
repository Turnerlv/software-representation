...
test('analyzeTarget does NOT emit false-positive OPEN_CONNECTORs for non-allowlisted calls', () => {
    ...
    assert.strictEqual(graph.openConnectors.length, 0, `Expected 0 open connectors but got ${graph.openConnectors.length}: ` +
...
test('analyzeTarget extracts Express Route Definition as CONTRACT', () => {
    ...
    assert.ok(graph.contracts.some((c) => c.name === 'Express Route: GET /'), 'Expected to extract Express GET / route as CONTRACT');
    assert.ok(graph.contracts.some((c) => c.name === 'Express Route: POST /login'), 'Expected to extract Express POST /login route as CONTRACT');
    assert.ok(graph.contracts.some((c) => c.name === 'Express Route: GET /items'), 'Expected to extract Express GET /items route from router as CONTRACT');
    assert.ok(graph.contracts.some((c) => c.name === 'Express Param: user'), 'Expected to extract Express Param: user as CONTRACT');
    assert.ok(graph.contracts.some((c) => c.name === 'Express Content Negotiation: application/json, default'), 'Expected to extract Express Content Negotiation: application/json, default as CONTRACT');
});
test('analyzeTarget extracts Express Response Connectors', () => {
    const fixtureDir = path.join(process.cwd(), '..', '..', 'fixtures', 'test-repos', 'express-app');
    const graph = analyzeTarget(fixtureDir);
    assert.ok(graph.openConnectors.some((c) => c.name === 'Express File Response: download'), 'Expected to extract Express File Response: download as OPEN_CONNECTOR');
    assert.ok(graph.openConnectors.some((c) => c.name === 'Express View Render'), 'Expected to extract Express View Render as OPEN_CONNECTOR');
    assert.ok(graph.openConnectors.some((c) => c.name === 'Express Redirect'), 'Expected to extract Express Redirect as OPEN_CONNECTOR');
    assert.ok(graph.openConnectors.some((c) => c.name === 'Express HTTP Response: send'), 'Expected to extract Express HTTP Response: send as OPEN_CONNECTOR');
    assert.ok(graph.openConnectors.some((c) => c.name === 'Express HTTP Response: json'), 'Expected to extract Express HTTP Response: json as OPEN_CONNECTOR');
    assert.ok(graph.openConnectors.some((c) => c.name === 'Express HTTP Response: sendStatus'), 'Expected to extract Express HTTP Response: sendStatus as OPEN_CONNECTOR');
});
test('analyzeTarget extracts Express Router Mount as RELATIONSHIP', () => {
    ...
    assert.ok(graph.relationships.some((r) => r.name === 'Express Mount: /api -> apiRouter'), 'Expected to extract Express /api mount as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name === 'Express Mount: /users -> usersRouter'), 'Expected to extract Express /users mount as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name === 'Express Mount: Root -> express.json()'), 'Expected to extract Express Root -> express.json() mount as RELATIONSHIP');
});
test('analyzeTarget extracts CommonJS require as RELATIONSHIP', () => {
    const fixtureFile = path.join(process.cwd(), '..', '..', 'fixtures', 'test-repos', 'express-app', 'app.js');
    const graph = analyzeTarget(fixtureFile);
    assert.ok(graph.relationships.some((r) => r.name === 'Require: express'), 'Expected to extract require("express") as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name === 'Require: ./routes/api'), 'Expected to extract require("./routes/api") as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name === 'Require: ./routes/users'), 'Expected to extract require("./routes/users") as RELATIONSHIP');
});
test('analyzeTarget extracts CommonJS Module Export as BOUNDARY', () => {
    const fixtureFile = path.join(process.cwd(), '..', '..', 'fixtures', 'test-repos', 'express-app', 'app.js');
    const graph = analyzeTarget(fixtureFile);
    assert.ok(graph.boundaries.some((b) => b.name === 'CJS Export: default'), 'Expected to extract module.exports as BOUNDARY');
    assert.ok(graph.contracts.some((c) => c.name === 'CJS Export: init'), 'Expected to extract app.init as CONTRACT');
    assert.ok(graph.contracts.some((c) => c.name === 'Dynamic Export: app[method]'), 'Expected to extract dynamic method assignment as CONTRACT');
    assert.ok(graph.contracts.some((c) => {
    ...
    assert.ok(graph.contracts.some((c) => c.name === 'Property Getter: protocol'), 'Expected to extract Object.defineProperty protocol getter as CONTRACT');
});
test('analyzeTarget extracts EventEmitter patterns', () => {
    const fixtureDir = path.join(process.cwd(), '..', '..', 'fixtures', 'test-repos', 'node-events');
    const graph = analyzeTarget(fixtureDir);
    assert.ok(graph.contracts.some((c) => c.name === 'Event Listener: event'), 'Expected to extract EventEmitter.on as CONTRACT');
    assert.ok(graph.openConnectors.some((r) => r.name === 'Event Emit: event'), 'Expected to extract EventEmitter.emit as OPEN_CONNECTOR');
});
test('analyzeTarget extracts Object.create as RELATIONSHIP', () => {
    ...
    const graph = analyzeTarget(fixtureFile);
    assert.ok(graph.relationships.some((r) => r.name === 'Inherits: http.IncomingMessage.prototype'), 'Expected to extract Object.create as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name === 'Mixes: EventEmitter'), 'Expected to extract prototype mixin as RELATIONSHIP');
});
...
test('analyzeTarget extracts dynamic require and import as RELATIONSHIP', () => {
    ...
    const graph = analyzeTarget(fixtureFile);
    assert.ok(graph.relationships.some((r) => r.name === 'Dynamic Require: mod'), 'Expected to extract require(mod) as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name === 'Dynamic Require: engine'), 'Expected to extract require(engine) as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name === 'Import: path'), 'Expected to extract import("path") as RELATIONSHIP');
    assert.ok(graph.relationships.some((r) => r.name.includes('Dynamic Import:')), 'Expected to extract dynamic import() as RELATIONSHIP');
});
