...
function mapRowToCoverageEntry(row) {
    return {
        id: row.id,
 ...
 */
export function logExtractionGap(db, gapData) {
    ...
    const status = gapData.status ?? 'DISCOVERED';
    const impactLevel = gapData.impactLevel ?? gapData.impact_level ?? 'MEDIUM';
    ...
    const evidenceSnippet = gapData.evidenceSnippet ?? gapData.evidence_snippet ?? null;
    const fixLocation = gapData.fixLocation ?? gapData.fix_location ?? null;
    const fixPatternSummary = gapData.fixPatternSummary ?? gapData.fix_pattern_summary ?? null;
    const testFixturePath = gapData.testFixturePath ?? gapData.test_fixture_path ?? null;
  ...
  `);
    stmt.run(id, patternName, framework, status, impactLevel, evidenceRepo, evidenceFile, evidenceLine, evidenceSnippet, fixLocation, fixPatternSummary, testFixturePath);
    const row = db
 ...
 */
export function resolveExtractionGap(db, gapId, fixDetails) {
    const fixLocation = fixDetails.fixLocation ?? fixDetails.fix_location;
    if (!fixLocation) {
        throw new Error('fixLocation (or fix_location) is required');
    }
    const fixPatternSummary = fixDetails.fixPatternSummary ?? fixDetails.fix_pattern_summary;
    if (!fixPatternSummary) {
        throw new Error('fixPatternSummary (or fix_pattern_summary) is required');
    }
    const testFixturePath = fixDetails.testFixturePath ?? fixDetails.test_fixture_path ?? null;
    const stmt = db.prepare(`
    ...
    WHERE id = ?
  `);
    const result = stmt.run(fixLocation, fixPatternSummary, testFixturePath, gapId);
    if (result.changes === 0) {
        return null;
    }
    const row = db
...
export function getLedgerSummary(db) {
    ...
    const resolvedRow = db
        .prepare("SELECT COUNT(*) as count FROM extractor_coverage_ledger WHERE status = 'RESOLVED'")
        .get();
    const resolvedCount = resolvedRow.count;
    const resolvedPercentage = totalPatterns === 0
        ? 0
    ...
    const statusRows = db
        .prepare('SELECT status, COUNT(*) as count FROM extractor_coverage_ledger GROUP BY status')
        .all();
    ...
    const frameworkRows = db
        .prepare('SELECT framework, COUNT(*) as count FROM extractor_coverage_ledger GROUP BY framework')
        .all();
    ...
    const impactRows = db
        .prepare('SELECT impact_level, COUNT(*) as count FROM extractor_coverage_ledger GROUP BY impact_level')
        .all();
 ...
 */
export function getAllLedgerEntries(db) {
    const rows = db
        .prepare('SELECT * FROM extractor_coverage_ledger ORDER BY created_at DESC, id DESC')
        .all();
