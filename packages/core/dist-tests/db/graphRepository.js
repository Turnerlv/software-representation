 ...
 */
export function saveRepresentationGraph(db, repo, graph) {
    ...
    `).run(repo.id, repo.name, repo.path, graph.analyzedAt, repo.extractorVersion ?? null, repo.commitSha ?? null);
        db.prepare('DELETE FROM structural_entities WHERE repository_id = ?').run(repo.id);
        ...
        for (const entity of allEntities) {
            ...
            for (const ev of evidences) {
                if (ev) {
                    ...
                    insertEvidence.run(evidenceId, globalEntityId, ev.filePath, ev.lineNumber ?? null, ev.snippet ?? null, ev.evidenceRole ?? null);
                }
            }
        }
    });
 ...
 */
export function getRepresentationGraph(db, repoId) {
    const repoRow = db
    ...
    const entityRows = db
        .prepare('SELECT id, name, type, source_id, target_id, status, confidence FROM structural_entities WHERE repository_id = ?')
        .all(repoId);
    ...
    for (const row of entityRows) {
        ...
        if (row.source_id)
            entity.sourceId = row.source_id;
        if (row.target_id)
            entity.targetId = row.target_id;
        if (row.status)
