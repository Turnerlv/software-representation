...
import { getAllLedgerEntries, getLedgerSummary, getRepresentationGraph, initDatabase, logExtractionGap, resolveExtractionGap, saveRepresentationGraph, } from './index.js';
...
test('extractor_coverage_ledger logExtractionGap, resolveExtractionGap, and getLedgerSummary', () => {
    ...
    assert.strictEqual(resolvedGap?.fixLocation, 'packages/core/src/extractors/express.ts:45');
    assert.strictEqual(resolvedGap?.fixPatternSummary, 'Added visitor for app.use router mounting');
    assert.strictEqual(resolvedGap?.testFixturePath, 'fixtures/unit/express.test.ts');
